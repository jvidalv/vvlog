...
export function ArticleSnippetWithImage(props) {
  ...
  return (
    <SnippetContainer
      className={
        'd-flex mb-3 ' + (props.className ?? '') + (fake ? ' empty' : '')
      }
      icon={props.icon}
      key={id}
    >
      <ImageSnippet
        className="d-none d-lg-block image-snippet"
...
export function ArticleSnippet(props) {
      ...
      if (position !== -1) {
        element.innerHTML = [
          ...
          `<span class="highlight">${content.slice(
            position,
            position + props.q.length
          )}</span>`,
          content.slice(position + props.q.length),
        ].join('');
      }
    } else {
  ...
  return (
    <SnippetContainer
      ref={snippetRef}
      className={
        props.className + (fake ? ' empty' : '') + ' simple text-center p-4'
      }
      icon={props.icon}
      key={id}
    >
